...
A non-invasive DeepSeek Harness Web plugin for a live high-school mathematics tutoring demonstration. It keeps the existing chat composer, Markdown, image attachment, and LaTeX response path, while adding a selected-problem card and a diagnosis-first teaching policy.

## What it demonstrates

- A selected examination-style mathematics problem is visible above the composer.
...
Restart `dsh web` after installation. The bundled configuration enables the default demonstration question.
...
The plugin adds a stable teaching-policy section and the selected problem to every model request. It requires the visible sections `思维诊断`, `下一步问题`, `最小提示`, and `教学依据`.
